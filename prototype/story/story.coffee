...
story = {
  intro: ->
    ...
    Location : Middle of an empty field . Early hours of Sunday morning. 
    ...
    @location loc.start, ->
      'look around/examine surroundings/analyse area': ->
        ...
        He's sitting by his tractor, parked a about 20 meters away towards the road.
    ...
    @everywhere ->
      ...
      'tractor/inspect tractor/examine tractor/go to tractor': ->
        ...
        if not @farmerStandsUp
          @text "The farmer sitting next to it stands up as the detective was approaching."
        @farmerStandsUp = true;
      ...
      'farmer/go to farmer/talk to farmer': ->
        @location loc.next_to_tractor
        @text "Willy walks to the farmer. Middle aged man, dirty hands, with a bit of a red face."
        if not @farmerStandsUp
